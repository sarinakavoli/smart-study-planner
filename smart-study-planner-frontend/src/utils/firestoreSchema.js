...
//
// Document ID: org_<nanoid(10)>  (or org_<uid> for personal orgs)
//
// Fields:
...
//
// Notes:
//   - Currently queried by userId only (single-user mode)
//   - To scope by org: add where("organizationId", "==", orgId) alongside
//     the existing where("userId", "==", uid) filter
//   - The readable document ID makes it easy to trace a category in logs
//     without having to look it up in the Firebase Console
//
//
// ── COLLECTION: tasks ────────────────────────────────────────────────────────
//
// Document ID: task_<orgId>_<userId>_<nanoid(10)>
