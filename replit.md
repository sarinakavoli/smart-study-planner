...
> Membership is fully handled via flat fields on `users/{uid}`.
...
```
  ...
  organizationId:   org the user belongs to (or null)
  organizationName: denormalized org name for display (or null)
  role:             "admin" | "teacher" | "student" | null
  departmentId:     department within the org (or null)
  departmentName:   denormalized dept name (or null)
  membershipStatus: "not_invited" | "pending" | "active"
  createdAt:        serverTimestamp()
  updatedAt:        serverTimestamp()
```

> No `organizations` array. No `uid` field. All membership is in flat fields.

### Organizations collection (`organizations/{organizationId}`)

```
  id:             organizationId
  name:           human-readable school/org name
...
### Departments subcollection (`organizations/{organizationId}/departments/{departmentId}`)

```
  id:         departmentId  (e.g. "dep_computer-science")
...
- ID generated by `generateDepartmentId(name)` in `firestoreIds.js` → `dep_<nameSlug>`
- Only active admins can create departments

### Role-based behavior

| Role    | Can create org | Can invite | Can manage departments | Can add/edit tasks |
|---------|---------------|------------|----------------------|-------------------|
| admin   | ✅             | ✅          | ✅                    | ✅                 |
| teacher | ❌             | ❌          | ❌                    | ✅                 |
| student | ❌             | ❌          | ❌                    | ✅                 |
...
2. Resolve `membershipStatus` from flat fields:
   - If `membershipStatus` is already set, use it.
   - Migration: if missing, infer `"active"` (has `organizationId + role`) or `"not_invited"`.
3. Write or refresh the user document:
   - New users → create with `organizationId: null`, `role: null`, `membershipStatus: "not_invited"`.
   - Returning users → merge-update `email`, `displayName`, `updatedAt` (and `membershipStatus` if migrating).
4. Route:
   - `membershipStatus === "active"` → `ALL_TASKS`
   - `membershipStatus === "pending"` → `PENDING_MEMBERSHIP`
   - `membershipStatus === "not_invited"` → `CREATE_ORG`

### Invite flow (flat membership)

- Admins see **Invite User** and **Departments** in the sidebar (only when `membershipStatus === "active"`).
- Admin invites by email → queries `users` collection by email → updates the target user's doc directly:
  `{ organizationId, organizationName, role, departmentId, departmentName, membershipStatus: "pending" }`
- Invited user signs in → `membershipStatus === "pending"` → routed to `PENDING_MEMBERSHIP` view.
- User clicks **Accept** → `handleAcceptMembership` flips `membershipStatus` to `"active"`.
- There is no Decline. Users can sign out instead.
- The invitee must already have a registered account before the admin can invite them.

### Organization creation flow

1. Create `organizations/{orgId}` with `id`, `name`, `createdBy`, `createdByEmail`, `createdAt`, `updatedAt`.
2. Update `users/{uid}` (merge):
   - `{ organizationId: orgId, role: "admin", departmentId: null, membershipStatus: "active" }`

> **No auto-org, no auto-admin.** Every user must explicitly create or accept an invitation to join an org.
