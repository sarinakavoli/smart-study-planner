...
- Only admins can create departments (enforced client-side; Firestore rule requires `hasActiveOrg`)
...
```
  ...
  departmentId:     target department (or null)
  organizationName: denormalized org name for display
  departmentName:   denormalized dept name for display
  invitedEmail:     normalized lowercase email of invitee
...
- Acceptance fields added on accept: `acceptedByUserId`, `acceptedAt`
- Decline fields added on decline: `declinedByUserId`, `declinedAt`
- Service: `src/services/invitationService.js` — `createInvitation`, `getPendingInvitationsForEmail`, `acceptInvitation`, `declineInvitation`
...
2. Resolve org and role from `users/{uid}.organizations`:
   ...
   - Use that entry's `role` and `status`. Only `status === "activated"` counts.
2.5. Query `invitations` for pending invitations where `invitedEmail == user.email`.
3. Write or refresh the user document:
   - New users → create with `organizations: []`, `organizationId: null`.
   - Returning users → merge-update `email`, `displayName`, `updatedAt` only.
4. Route:
   - If has active org → `ALL_TASKS`
   - If no active org + pending invitations → `PENDING_INVITATIONS`
   - If no active org + no invitations → `CREATE_ORG`

### Invitation flow

- Admins see **Invite User** and **Departments** in the sidebar.
- Admin sends invitation → writes `invitations/{id}` with status `pending`.
- Invited user signs in → step 2.5 loads their pending invitations into state.
- User is shown `PENDING_INVITATIONS` view with Accept / Decline buttons.
- **Accept**: appends `{ organizationId, departmentId, role, status:"activated" }` to `users/{uid}.organizations`, sets `organizationId` if not already set, marks invitation `accepted`.
- **Decline**: marks invitation `declined`. No changes to `users/{uid}`.
- **Nothing is auto-accepted.** The user must explicitly click Accept.
...
2. Update `users/{uid}` (merge):
   ...
   - `arrayUnion` `{ organizationId, departmentId: null, role: "admin", status: "activated" }`
